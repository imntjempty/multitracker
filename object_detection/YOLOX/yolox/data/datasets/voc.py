...
class VOCDetection(Dataset):
    ...
    def __init__(
        ...
        self.ids = list()
        for mode in image_sets:
            self.mode = mode 
            rootpath = self.root
            for line in open(
                os.path.join(rootpath, "ImageSets", "Main", "%s.txt" % mode) # dolokov
            ):
    ...
    def _get_voc_results_file_template(self):
        ...
        filedir = os.path.join(self.root, "results", self.mode, "Main") # dolokov
    ...
    def _do_python_eval(self, output_dir="output", iou=0.5):
        rootpath = self.root# os.path.join(self.root, self.mode)
        name = self.image_set[0][1]
        annopath = os.path.join(rootpath, "Annotations", "{:s}.xml")
        imagesetfile = os.path.join(rootpath, "ImageSets", "Main", name + ".txt")
        cachedir = os.path.join(
            self.root, "annotations_cache", self.mode, name
        )
        if not os.path.exists(cachedir):
            os.makedirs(cachedir)
        aps = []
        # The PASCAL VOC metric changed in 2010
        use_07_metric = True# if int(self._year) < 2010 else False # dolokov
